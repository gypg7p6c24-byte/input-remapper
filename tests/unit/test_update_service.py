...
class TestUpdateService(unittest.TestCase):
    def test_normalize_version(self):
        self.assertEqual(normalize_version("2.3.2~dev1"), "2.3.2.dev1")
    ...
    def test_update_release_version_comparisons(self):
        release = UpdateRelease(
            channel="dev",
            version="2.3.2.dev1",
            debian_version="2.3.2~dev1",
            release_url="https://example.invalid/release",
            asset_name="input-remapper-2.3.2~dev1.deb",
            asset_url="https://example.invalid/input-remapper-2.3.2~dev1.deb",
        )
        self.assertTrue(release.is_newer_than("2.3.1"))
        self.assertTrue(release.differs_from("2.3.1"))
        self.assertFalse(release.is_older_than("2.3.1"))
    ...
    def test_fetch_release_parses_deb_asset(self):
        payload = {
            ...
            "assets": [
                {
                    "name": "input-remapper-2.3.2~dev1.deb",
                    "browser_download_url": "https://example.invalid/input-remapper-2.3.2~dev1.deb",
                }
            ],
        }
        ...
        self.assertEqual(release.version, "2.3.2.dev1")
        self.assertEqual(release.debian_version, "2.3.2~dev1")
